...
NATURE_COLORS = ["#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7", "#999999"]
ELSEVIER_COLORS = ["#4477AA", "#CC6677", "#DDCC77", "#117733", "#AA4499", "#88CCEE", "#332288", "#44AA99"]
MDPI_COLORS = ["#006633", "#FF6600", "#0066CC", "#CC0033", "#660066", "#FFCC00", "#339933", "#993300"]
...
class SCIFigures:
    ...
    def figure1_study_area(self, satellite_img, boundary_gdf, inset_map=None, title=None):
        ...
        if satellite_img is not None:
            ax.imshow(satellite_img, cmap="gray")
        if boundary_gdf is not None:
            boundary_gdf.boundary.plot(ax=ax, color="red", linewidth=0.8)
        ...
        if inset_map is not None:
            ...
            ax_inset.imshow(inset_map)
            ax_inset.set_title("(a)", fontsize=6)
            ax_inset.axis("off")
    ...
    def figure2_land_cover(self, lc_maps, time_labels, class_names=None):
        ...
        for i, (m, lbl) in enumerate(zip(lc_maps, time_labels)):
            ...
            ax.set_title(lbl, fontweight="bold", fontsize=8)
            ax.axis("off")
        if class_names:
            lax = fig.add_axes([0.1, 0.02, 0.8, 0.03])
            lax.axis("off")
    ...
    def figure4_change_detection(self, change_map, transition_matrix, class_names=None, period="2010-2020"):
        fig, (ax1, ax2) = plt.subplots(
            1,
            2,
            figsize=(self.config["full_width"] * 0.03937, self.config["full_width"] * 0.03937 * 0.45),
            gridspec_kw={"width_ratios": [1.2, 1]},
        )
        ax1.imshow(change_map, cmap=ListedColormap(["#CCCCCC", "#FF6B6B", "#4ECDC4"]), interpolation="nearest")
        ax1.set_title(f"Change Map ({period})", fontweight="bold")
        ax1.axis("off")
        ...
        for i in range(tm.shape[0]):
            for j in range(tm.shape[1]):
                ax2.text(
                    ...
                    ha="center",
                    va="center",
                    color="white" if tm[i, j] > tm.max() / 2 else "black",
                    fontsize=5,
                )
        if class_names:
        ...
        ax2.set_xlabel("To", fontsize=7)
        ax2.set_ylabel("From", fontsize=7)
    ...
    def figure5_spatial_pattern(self, hotspot_map, lisa_map, moran_scatter=None):
        ...
        ax1.imshow(
            hotspot_map,
            cmap=ListedColormap(["#053061", "#2166AC", "#92C5DE", "#F7F7F7", "#F4A582", "#D6604D", "#B2182B"]),
            interpolation="nearest",
            vmin=-3,
            vmax=3,
        )
        ax1.set_title("Hotspot (Gi*)", fontweight="bold")
        ax1.axis("off")
        ax2 = fig.add_subplot(gs[1])
        ax2.imshow(
            lisa_map,
            cmap=ListedColormap(["#F7F7F7", "#FF0000", "#0000FF", "#FF69B4", "#87CEEB"]),
            interpolation="nearest",
        )
        ax2.set_title("LISA Cluster", fontweight="bold")
        ax2.axis("off")
        if moran_scatter:
            ax3 = fig.add_subplot(gs[2])
            ax3.scatter(moran_scatter["values"], moran_scatter["spatial_lag"], alpha=0.5, s=10, c="#4477AA")
            ax3.set_xlabel("Value")
            ax3.set_ylabel("Spatial Lag")
            ax3.set_title("Moran Scatter", fontweight="bold")
    ...
    def figure6_driver_analysis(self, feature_importance, feature_names=None):
        ...
        imp = np.array(feature_importance)
        names = feature_names or [f"F{i+1}" for i in range(len(imp))]
        idx = np.argsort(imp)
        ax.barh(range(len(idx)), imp[idx], color=NATURE_COLORS[: len(idx)])
        ax.set_yticks(range(len(idx)))
        ax.set_yticklabels([names[i] for i in idx], fontsize=6)
    ...
    def figure7_uncertainty(self, uncertainty_map):
        fig, ax = plt.subplots(
            figsize=(self.config["full_width"] * 0.03937, self.config["full_width"] * 0.03937 * 0.35)
        )
        im = ax.imshow(uncertainty_map, cmap="RdYlGn_r", interpolation="bilinear")
        ax.set_title("Uncertainty", fontweight="bold")
        ax.axis("off")
    ...
    def figure8_framework(self, steps):
        ...
        for i, (p, s) in enumerate(zip(pos, steps)):
            box = FancyBboxPatch(
                ...
                facecolor=NATURE_COLORS[i % len(NATURE_COLORS)],
                edgecolor="black",
                alpha=0.9,
            )
            ax.add_patch(box)
            ax.text(p, 0.5, s, ha="center", va="center", fontsize=6, fontweight="bold")
            if i < n - 1:
                ax.annotate(
                    "",
                    xy=(pos[i + 1] - 0.08, 0.5),
    ...
    def compose_figure_grid(self, image_paths, layout="2x2", labels=None, output="Composed.png"):
        ...
        fig, axes = plt.subplots(
            rows, cols, figsize=(self.config["full_width"] * 0.03937, self.config["full_width"] * 0.03937 * 0.6)
        )
