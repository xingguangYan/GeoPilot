...
def main():
    ...
    subparsers.add_parser("env", help="Check environment")
    ...
    if args.command == "env":
        ...
        print(json.dumps(generate_env_report(), indent=2))
    elif args.command == "scan":
        from geoai_data_manager import DataManager

        dm = DataManager()
        found = dm.scan_directory(args.path)
        ...
        for tier, js in recs.items():
            if js:
                print(f"\n{tier.upper()}:")
            for j in js:
                print(f"  {j['name']} (IF={j['if']}, AR={j['accept_rate']}%)")
        ...
        if wf:
            result = wf("Region", [2010, 2020])
            print(result)
        else:
            print(f"Pipeline {args.name} not found")
    elif args.command == "report":
        from geoai_report import generate_report
